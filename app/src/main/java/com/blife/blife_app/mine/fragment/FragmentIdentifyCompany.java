...
import com.blife.blife_app.mine.api.API_GetCompanyinfo;
import com.blife.blife_app.mine.api.API_IdentifyCitizen;
import com.blife.blife_app.mine.api.API_IdentifyCitizenInfo;
import com.blife.blife_app.mine.api.API_IdentifyCompany;
import com.blife.blife_app.mine.api.API_IdentifyCompanyInfo;
 ...
 */
public class FragmentIdentifyCompany extends BaseFragment implements View.OnClickListener, InterfacePermissionResult {
    ...
    private TextView company_tv_identify_fail;
    ...
    private void initGetCompanyInfo() {
        API_GetCompanyinfo api_getCompanyinfo = new API_GetCompanyinfo(ACCESS_TOKEN);
        dataManager.getServiceData(api_getCompanyinfo);
    }
    ...
    private void initView() {
        ...
        //其他
        company_tv_identify_fail = (TextView) rootView.findViewById(R.id.company_tv_identify_fail);
    ...
    public int getLayout() {
        return R.layout.fragment_identify_company;
    }
    ...
    private void identifyCommit() {
        ...
        if (TextUtils.isEmpty(card)) {
            showFailedDialog(R.string.uploadadv_empty_company_card, identify_tv_commit);
            return;
        }
        ...
        API_IdentifyCompanyInfo api_identifyCompanyInfo = new API_IdentifyCompanyInfo(ACCESS_TOKEN, json);
        dataManager.getServiceData(api_identifyCompanyInfo);
...
//                    ImageLoader.getInstance().loadImage(beanUploadImage.getAdvimg(), addPicImageview, true);
                ...
                if (tag.equals(API_IdentifyCompanyInfo.TAG)) {
                    LogUtils.e("先是企业身份认证****" + json);
                    API_IdentifyCompany api_identifyCompany = new API_IdentifyCompany(ACCESS_TOKEN);
                    dataManager.getServiceData(api_identifyCompany);
                }
                if (tag.equals(API_IdentifyCompany.TAG)) {
                    LogUtils.e("最后提交认证****" + json);
                    cancelLoadingDialog();
                    getActivity().finish();
                }
                if (tag.equals(API_GetCompanyinfo.TAG)) {
                    cancelLoadingDialog();
                    LogUtils.e("获取企业信息 ****" + json);
                    if (TextUtils.isEmpty(json)) {
                        JsonObjUItils.isEmptyJson(getActivity());
                        return;
                    }
                    ...
                    if (beanIdentify.getVerify_result() != null) {
                        ...
                        if (verify_result.getVerify_status() == 0) {
                            company_tv_identify_fail.setVisibility(View.GONE);
                        } else if (verify_result.getVerify_status() == 1) {
                            company_tv_identify_fail.setVisibility(View.GONE);
                        } else if (verify_result.getVerify_status() == 2) {
                            company_tv_identify_fail.setVisibility(View.GONE);
                        } else if (verify_result.getVerify_status() == 3) {
                            company_tv_identify_fail.setVisibility(View.VISIBLE);
                            if (verify_result.getVerify_reason() != null)
                                company_tv_identify_fail.setText("注:" + verify_result.getVerify_reason());
                        } else {
                            company_tv_identify_fail.setVisibility(View.GONE);
                        }
                    }
                    if (beanIdentify.getInfo().getExtra_content() != null && beanIdentify.getInfo().getExtra_content().getImages() != null && beanIdentify.getInfo().getExtra_content().getImages().size() >= 2) {
                        BitmapUtils bitmapUtils = BitmapHelp.getBitmapUtils(getActivity());
                        bitmapUtils.display(identify_iv_face, beanIdentify.getInfo().getExtra_content().getImages().get(0));
                        bitmapUtils.display(identify_iv_inverse, beanIdentify.getInfo().getExtra_content().getImages().get(1));
            ...
            public void onError(Object tag, String message) {
                cancelLoadingDialog();
                if (tag.equals(API_UploadImage.TAG)) {
                    JsonObjUItils.getJsonCode(message, identify_fl_face, getActivity(), R.string.get_filer);
                } else if (tag.equals(API_IdentifyCitizenInfo.TAG)) {
                    JsonObjUItils.getJsonCode(message, identify_fl_face, getActivity(), R.string.get_filer);
                } else if (tag.equals(API_IdentifyCitizen.TAG)) {
                    JsonObjUItils.getJsonCode(message, identify_fl_face, getActivity(), R.string.get_filer);
                }
            }
        }
