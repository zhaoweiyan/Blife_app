 ...
 */
public class AdapterMyadvPrepare extends RecyclerView.Adapter {

    private Context context;
    private List<BeanMyadv> list;

    public AdapterMyadvPrepare(Context context, List<BeanMyadv> list) {
...
//                ImageLoader.getInstance().loadImage(images.get(0), myadvViewHolder.iv_advdimages, true);
        ...
        LogUtils.e("status****" + beanMyadv.getStatus());
        myadvViewHolder.lin_list.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (beanMyadv.getStatus().trim().equals("1")) {
                    Intent intent = new Intent();
                    Bundle bundle = new Bundle();
                    bundle.putString(Constants.CREATE_ADV_ID, beanMyadv.getAdv_id());
                    bundle.putInt(Constants.ADV_CURRENT_TYPE, Constants.ADV_CURRENT_TYPE_DRAFT);
                    intent.putExtras(bundle);
                    intent.setClass(context, ActivitySendAdv.class);
                    ...
                    intent.putExtras(bundle);
                    intent.setClass(context, ActivityMyadvPrepareDetail.class);
                    context.startActivity(intent);
                }
